...
class Lumos_train:
    ...
    # Here we estimate photometry on CPUs. This should be much
    # simpler to integrate and sufficiently fast.
    def __init__(self, model_path = None, batch_size=500, pretrained = False, data_dir, stamp_shape=(60, 60), epochs,model_outdir):
    ...
    for epoch in range(self.epochs):
        ...
        for meta, prof_conv, stamp, field in loader_train:
            ...
            flux, logalpha,logsig = self.cnn(stamp,  band.cuda(),coord.cuda(),Iauto.cuda(),n.cuda())
        ...
        print('Trainig - epoch %s:'%epoch, np.median(loss))
        ...
        net.eval()
        for meta, prof_conv,  stamp, field  in loader_test:
            ...
            log_prob =   logalpha - 0.5*(flux - lab.cuda()).pow(2) / sig.pow(2) - logsig - logerf.cuda()
            log_prob = torch.logsumexp(log_prob, 1)
            loss = -log_prob.mean()

        loss = loss.detach().cpu().numpy()
        print('flux',flux,'true',lab, 'error',sig)
        print('Test - epoch %s:'%epoch, np.median(loss)) 
        #chi2 = (flux.detach().cpu()-lab)/sig.detach().cpu()
        #print(chi2.mean(), chi2.std())
